...
function mountPulseSphere(canvas, { particles = 3600, still = false, time = 2.1, offsetX = 0, offsetY = 0, scale = 1 } = {}) {
    ...
    const draw = (t) => {
        ...
        for (const s of strands) {
            ...
            for (let i = 0; i <= segs; i++) {
                ...
                const y1 = y * ctx2 - z * stx;
                z = y * stx + z * ctx2;
                y = y1;
                const x1 = x * ctz - y * stz;
                y = x * stz + y * ctz;
                x = x1;
            ...
            for (const pass of [0, 1]) {
                for (let i = 0; i < segs; i += chunk) {
                    ...
                    for (let j = i + 1; j <= Math.min(segs, i + chunk); j++)

...
export default async function Discover({ searchParams }: { searchParams: SP }) {
  ...
  return (
    <div>
      <PageHeader title={t("Descubrir")} subtitle={t("Citas reales: perfiles verificados compatibles con tus preferencias de género, prototipo, intereses y personalidad.")} />
      ...
      <form className="card mb-8 grid gap-3 md:grid-cols-6" method="get">
        <select name="archetype" defaultValue={filters.archetype ?? ""} className="input">
          <option value="">{t("Todos los prototipos")}</option>
        ...
        <select name="intent" defaultValue={filters.intent ?? ""} className="input">
          <option value="">{t("Cualquier intención")}</option>
          {INTENTS.map((i) => <option key={i.id} value={i.id}>{t(i.label)}</option>)}
        </select>
        <select name="net_worth" defaultValue={filters.netWorth ?? ""} className="input" disabled={tier.rank < 1} title={tier.rank < 1 ? t("Disponible desde Gold") : ""}>
          <option value="">{t("Patrimonio")} {tier.rank < 1 ? "(Gold+)" : ""}</option>
          {NET_WORTH.filter((n) => n.id !== "na").map((n) => <option key={n.id} value={n.id}>{t(n.label)}</option>)}
        </select>

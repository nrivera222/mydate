    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div className="flex-1">
          <Link href={`/perfil/${other.user_id}`} className="font-display text-2xl hover:text-glow">{other.name}</Link>
      ...
      <div className="card min-h-[320px] space-y-3">
        ...
        {msgs.map((m) => (
          <div key={m.id} className={`flex ${m.from_id === user.id ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[75%] rounded-2xl px-4 py-2 text-sm ${m.from_id === user.id ? "bg-brand text-ink" : "bg-ink-3"}`}>
      ...
      <form action={sendMessage} className="mt-4 flex gap-2">
        ...
        <button className="btn-brand" type="submit">{t("Enviar")}</button>
      </form>

// Logotipo TWO LOVE renderizado con el efecto "Pulse Sphere": la composición del logo
// (esfera, latido y dos órbitas entrelazadas + wordmark) construida con partículas luminosas,
// plasma, filamentos de luz, niebla y brillo aditivo. Canvas 2D, sin dependencias.
// Bucle perfecto de LOOP segundos (8 latidos) precedido de una intro opcional de ensamblado.

export type PulseLogoLayout = "symbol" | "horizontal" | "stacked";

export type PulseLogoOptions = {
  layout?: PulseLogoLayout;
  /** Imagen del wordmark (SVG "TWO LOVE"); no se usa en layout "symbol". */
  wordmark?: HTMLImageElement | null;
  /** Imagen del claim bajo el wordmark (solo layout "stacked"). */
  tagline?: HTMLImageElement | null;
  particles?: number;
  /** Fondo transparente (PNG con alfa); si no, fondo espacial con niebla y estrellas. */
  transparent?: boolean;
  /** Reproducir la intro de ensamblado al comienzo (segundos 0…INTRO). */
  intro?: boolean;
  /** Render estático del instante `time` (sin bucle de animación). */
  still?: boolean;
  time?: number;
  /** Ocupación del lienzo por el logotipo (0–1). */
  fill?: number;
  /** Desplazamiento del logotipo (fracción del ancho/alto), para componer piezas con texto. */
  offsetX?: number;
  offsetY?: number;
};

export type PulseLogoHandle = { draw: (t: number) => void; resize: () => void; destroy: () => void };

export const LOOP = 10; // s, múltiplo exacto del latido (1,25 s × 8)
export const INTRO = 2.6; // s
...
const heartbeat = (u: number) => 0.16 * gauss(u, -0.42, 0.07) - 0.22 * gauss(u, -0.09, 0.03) + gauss(u, 0, 0.035) - 0.5 * gauss(u, 0.09, 0.035) + 0.26 * gauss(u, 0.4, 0.09);
const beat = (t: number) => {
...
export function mountPulseLogo(canvas: HTMLCanvasElement, opts: PulseLogoOptions = {}): PulseLogoHandle {
  ...
  // Esfera de partículas (Fibonacci) con posición de partida dispersa para la intro
  ...
  // Órbitas: la misma geometría que el logotipo vectorial
  const rings = [
  ...
  // Maquetación: caja del símbolo (lado = 2.9 R) y rectángulos de wordmark/claim
  const layoutBoxes = () => {
    const l = layoutRaw(), dx = offsetX * w, dy = offsetY * h;
    const mv = (r: { x: number; y: number; w: number; h: number } | null) => (r ? { ...r, x: r.x + dx, y: r.y + dy } : null);
  ...
  const layoutRaw = () => {
    ...
    if (layout === "horizontal") {
      // símbolo B + hueco 0.12B + wordmark de alto 0.17B
  ...
  const draw = (tRaw: number) => {
    const t = intro ? tRaw - INTRO : tRaw; // t < 0 durante la intro
    const lt = ((t % LOOP) + LOOP) % LOOP; // tiempo dentro del bucle
    ...
    if (!transparent) {
      ...
      // Niebla volumétrica alrededor del símbolo
    ...
    // Órbitas: punto de un anillo en el ángulo a
    const ringPt = (r: (typeof rings)[number], a: number) => {
    ...
    const drawRing = (i: number, front: boolean) => {
      const r = rings[i];
      ...
      for (const pass of front ? [0, 1, 2] : [2]) {
        ...
        for (let k = 0; k < segs; k++) {
          const f0 = k / segs;
          if ((front ? f0 : 1 - f0) > ringDraw) continue;
    ...
    // Estela luminosa y nodo que recorren cada órbita (una vuelta por bucle)
    const drawComet = (i: number, frontPass: boolean) => {
      if (pIntro < 1) return;
      const r = rings[i];
    ...
    // Esfera de partículas: ensamblado desde la dispersión, profundidad de campo y centelleo
    for (const p of shell) {
      const e = ease((pIntro - p.delay) / (1 - 0.35));
      const rr = p.j * pulse;
      const [x, y, z] = rot(p.ox + (p.x * rr - p.ox) * e, p.oy + (p.y * rr - p.oy) * e, p.oz + (p.z * rr - p.oz) * e);
      const pr = proj(x, y, z);
      const depth = (1 - clamp01((z + 1) / 2)); // 1 delante
      ...
      // Color según la diagonal del logotipo (azul arriba-izquierda → magenta abajo-derecha)
    ...
    // Latido: trazo base luminoso + barrido de monitor sincronizado con el pulso
    ...
    if (ecgA > 0) {
      ...
      const reveal = ecgA; // se dibuja de izquierda a derecha en la intro
      const sweep = t < 0 ? -2 : -1 + 2 * ((((lt / 1.25 - 0.08 + 0.5) % 1) + 1) % 1);
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      for (let k = 0; k < N; k++) {
        const p0 = pts[k], p1 = pts[k + 1];
        if ((p0.u + 1) / 2 > reveal) break;
        const edge = 1 - Math.abs(p0.u) ** 3; // se desvanece en los extremos
        const col = p0.u < 0 ? mix(PALETTE[0], [255, 255, 255], 1 + p0.u) : mix([255, 255, 255], PALETTE[6], p0.u);
        const d = sweep - p0.u;
        const glow = d >= 0 && d < 0.9 ? (1 - d / 0.9) ** 2 : 0;
        for (const [wid, al] of [[13, 0.08 + 0.22 * glow], [5, 0.28 + 0.4 * glow], [1.9, 0.75 + 0.25 * glow]] as const) {
    ...
    // Polvo luminoso en órbita (bokeh); se omite con fondo transparente para no cortarse en los bordes
    ...
    // Wordmark y claim (aparecen al final de la intro con un leve desplazamiento)
    ctx.globalCompositeOperation = "source-over";
    ...
    if (wm && wordmark && wmA > 0) {
      ...
      // Brillo suave detrás del texto, al ritmo del latido
      ctx.globalCompositeOperation = "lighter";
  ...
  const ro = new ResizeObserver(() => {
    resize();
    if (still || reduced) draw(still ? time : INTRO + 0.4);
  });
  ro.observe(canvas);
  if (still || reduced) {
...
// Las órbitas se trazan durante la intro (0,25 → 0,8 del progreso)
function ringA(p: number) {
